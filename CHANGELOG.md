...
- Add `ColorQuantization`, the one place an 8-bit channel becomes a normalized float and back: `ToNormalized` decodes, `ToByte` encodes to the nearest channel, and `ToThresholdByte` gives the inclusive bound for a float cutoff, exact at every boundary. See [8-bit Channels and Normalized Floats](./docs/features/utilities/math-and-extensions.md#8-bit-channels-and-normalized-floats) ([#466](https://github.com/Ambiguous-Interactive/unity-helpers/issues/466)).
