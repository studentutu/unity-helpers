...
# SIGPIPE, and the pipeline reports 141 from a SUCCESSFUL match (#465). The list is
# newline-separated, so a whole-line match is a substring test on a newline-padded copy.
contains_line() {
    local haystack="$1"
    local needle="$2"
    [[ $'\n'"$haystack"$'\n' == *$'\n'"$needle"$'\n'* ]]
