...
- **A rectangular array's dimensions are bounded individually, not only by their product**: an **empty** rectangular array whose other axis exceeds `SerializationCapacityLimits.MaximumRestoredCapacity` (default 1,048,576) no longer deserializes — raise that limit if you persist one. Arrays that carry elements are unaffected, at any limit. `WProtoRectangular.TryAcceptShape` takes the largest stated axis as a fifth argument; generated code is regenerated with the analyzer that ships beside it, so only a hand-written caller is affected ([#437](https://github.com/Ambiguous-Interactive/unity-helpers/issues/437)).
...
- **A watcher on a non-`GameObject` type no longer loads every imported prefab**, which is where `SendMessage cannot be called during Awake, CheckConsistency, or OnValidate` came from: loading a prefab runs `OnValidate` on every component in it, and the load was only ever used to evaluate a type test. Watching a prefab by what it contains is not supported, so a sub-asset nested inside a `.prefab` no longer matches a watcher on its type; nested sub-assets in `.asset` files are unaffected ([#280](https://github.com/Ambiguous-Interactive/unity-helpers/issues/280)).
